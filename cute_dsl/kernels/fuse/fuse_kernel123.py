...
Grid: (NT/4, H, B)
Block: 768 threads (24 warps), warp-specialized with setmaxnreg (all groups 4-aligned):
  Warps 0-7:   TMA+K1 fused (8×1, vec4, prefetch pipeline) – 2 WGs, 128 regs
  Warps 8-19:  K2 MMA compute (10 active + 2 idle for WG alignment) – 3 WGs, 72 regs
  Warps 20-23: Store/Inversion warps – 1 WG, 32 regs

Pipeline (prefetch overlap):
  Warps 0-7:   prefetch chunk 0→stage 0, then loop:
                  TMA next chunk (warp 0), wait cur chunk, K1 compute, arrive(k1_done)
  Warps 8-19:  wait(k1_done) + wait(store_done) + wait(tma), MMA, arrive(mma_done + stage_reuse)
  Warps 20-23: wait(mma_done), store sAqk/sAkk→GMEM, arrive(store_done)
...
Mbarriers:
  ...
  k1_done_mbars[2]:      count=256, K1(8 warps) → MMA waits for g_cumsum ready
...
NUM_K1_TMA_WARPS = 8     # Warps 0-7:   fused TMA + K1 (2 warpgroups)
NUM_MMA_WARPS = 12        # Warps 8-19:  MMA (3 warpgroups, 10 active + 2 idle)
NUM_MMA_ACTIVE = 10       # only 10 warps do actual MMA work
NUM_STORE_WARPS = 4       # Warps 20-23: Store/Inversion (1 warpgroup)
NUM_WARPS = NUM_K1_TMA_WARPS + NUM_MMA_WARPS + NUM_STORE_WARPS  # 24
THREADS = NUM_WARPS * 32  # 768
...
PARTIAL_COLS_PER_WARP = K_DIM // NUM_K1_TMA_WARPS  # 16
...
# WG0-1 (warps 0-7):   K1+TMA  → setmaxregister_increase(128)
# WG2-4 (warps 8-19):  MMA     → setmaxregister_decrease(72)
# WG5   (warps 20-23): Store   → setmaxregister_decrease(40)
# Total: (2×96 + 3×88 + 1×40) × 128 threads/WG = 63488 ≤ 65536
NUM_REGS_K1 = 96
NUM_REGS_MMA = 88
NUM_REGS_STORE = 40
...
def k1_internal_barrier(*, loc=None, ip=None):
    """Named barrier for K1+TMA warps (0-7, 256 threads). barrier_id=2."""
    llvm.inline_asm(
        T.i32(), [],
...
def fused_kernel123(
    ...
    # =================================================================
    # Warps 0-7: Fused TMA + K1 (8×1 layout, vec4, with TMA prefetch)
    #
    #              cols 0-127
    # rows 0-7      warp 0  (also TMA issuer)
    # rows 8-15     warp 1
    # ...
    # rows 56-63    warp 7
    # =================================================================
    ...
    # =================================================================
    # Warps 8-19: K2 MMA Compute (10 active + 2 idle for WG alignment)
    # =================================================================
    if warp_idx >= NUM_K1_TMA_WARPS and warp_idx < NUM_K1_TMA_WARPS + NUM_MMA_WARPS:
    ...
    # =================================================================
    # Warps 20-23: Store/Inversion warps
    # =================================================================
    if warp_idx >= NUM_K1_TMA_WARPS + NUM_MMA_WARPS:
    ...
    try:
        ...
        if cubin_files:
            ...
            if result.returncode == 0:
                sass_file = os.path.join(script_dir, "fuse_kernel123.sass")
