...
Block: 736 threads (23 warps), warp-specialized with mbarrier pipeline:
  ...
  Warps 9-12:  Store/Inversion warps (consume sAqk/sAkk from MMA, do inversion + GMEM write)
  Warps 13-22: K2 MMA compute (10 warps, 1 tile per warp, full K=128 iteration)
...
def fused_kernel123(
    ...
    #   store_stage_mbars[2]: MMA warps arrive → Warp 0 (stage reuse)
    #   k1_done_mbars[2]:     K1 warps (1-8) → MMA warps (g_cumsum ready)
    #   mma_done_mbars[2]:    MMA warps (13-22) → Store warps (sAqk/sAkk ready)
    ...
    if tidx == 0:
        for s in range(NUM_STAGES):
            ...
            # Only MMA (10 warps) signal stage reuse
            cute.arch.mbarrier_init(store_stage_mbars + s, NUM_MMA_WARPS * 32)
            # K1 (8 warps) signal g_cumsum ready
    ...
    #   init=Store(4)*32=128, pre-arrive by first 4 MMA warps (13-16)=128 ✓
