...
Grid: (NT/4, H, B)
Block: 736 threads (23 warps), warp-specialized with mbarrier pipeline:
  Warp 0:      TMA producer (independent loop, double-buffered stages)
  Warps 1-8:   K1 compute (8×1: 8 row groups × 1 col group, vec4, 256B/warp store)
  Warps 9-12:  Store A_qk, A_kk (consumer of TMA, no K1 dependency)
  Warps 13-22: K2 MMA compute (TODO)
Mbarriers: tma_mbars[2], store_stage_mbars[2] (count=384, warps 1-12)
...
NUM_STORE_WARPS = 4
NUM_MMA_WARPS = 10
NUM_WARPS = 1 + NUM_K1_WARPS + NUM_STORE_WARPS + NUM_MMA_WARPS  # 23
THREADS = NUM_WARPS * 32  # 736
...
VEC = K1_COLS_PER_WARP // 32  # 4 (vec4 bf16 = 8B per thread, 256B per warp)
K_VEC = K_DIM // VEC          # 32
NUM_STAGES = 2
PARTIAL_COLS = K_DIM + 4      # 132, padded 4 cols to avoid bank conflicts in prefix sum
PARTIAL_COLS_PER_WARP = K_DIM // NUM_K1_WARPS  # 16 cols per warp in prefix phase
...
def store_internal_barrier(*, loc=None, ip=None):
    """Named barrier for store warps (9-12, 128 threads). barrier_id=1."""
    llvm.inline_asm(
        T.i32(), [],
...
def k1_internal_barrier(*, loc=None, ip=None):
    """Named barrier for K1 warps (1-8, 256 threads). barrier_id=2.
...
def fused_kernel123(
    ...
    tiled_copy_qk,                # TiledCopy vec2 for store warps (64 cols)
    tiled_copy_qk_k1,             # TiledCopy vec4 for K1 warps (128 cols)
    ...
    # Mbarrier protocol:
    #   tma_mbars[2]:         Warp 0 → Warps 1-12 (consumers wait for TMA data)
    #   store_stage_mbars[2]: Warps 1-12 (384 threads arrive) → Warp 0 (stage reuse)
    ...
    # Pre-arrive: warps 1-12 (K1 + store) flip phase
    ...
    # rows 56-63    warp 7
    #
    # Each warp: 8 rows × 128 cols, 32 threads × vec4
    # GMEM store: 32 threads × 8B (bf16×4) = 256 bytes = 2 sectors
    ...
    if warp_idx >= 1 and warp_idx < 1 + NUM_K1_WARPS:
        ...
        col_vec_idx = warp_col_group * (K1_COLS_PER_WARP // VEC) + lane_id  # 0..31
        ...
        for chunk_iter in cutlass.range_constexpr(CHUNKS_PER_BLOCK):
            ...
            # ---- Barrier: sync warps 1-8 (256 threads) ----
            k1_internal_barrier()

            # ---- Shuffle-based prefix sum across 8 row groups ----
            # Each of 8 warps handles 16 columns, 4 iterations of 4 cols × 8 rows
            ...
            for ri in cutlass.range_constexpr(ROWS_PER_K1_WARP):
                ...
                # Load K from swizzled SMEM via tiled_copy (vec4, full 128 cols)
                ...
                # Load Q from swizzled SMEM via tiled_copy (vec4, full 128 cols)
    ...
    # =================================================================
    # Warps 13-22: K2 MMA Compute (idle for now)
    # =================================================================
...
def make_host_function(B, NT, H):
    ...
    def host_fn(mQ, mK, mG, mA_log, mBeta, scale,
        ...
        # Tiled copy for K1 warps: vec4 from swizzled SMEM (32 threads × vec4 bf16)
        copy_atom_qk_k1 = cute.make_copy_atom(
            cute.nvgpu.CopyUniversalOp(),
            cutlass.BFloat16,
            num_bits_per_copy=64  # 4 x bf16 = 64 bits
        )
        tiled_copy_qk_k1 = cute.make_tiled_copy_tv(
            copy_atom_qk_k1,
            thr_layout=cute.make_layout((1, 32)),  # 32 threads along cols
            val_layout=cute.make_layout((1, 4))    # 4 values per thread
        )

        # Tiled copy for store warps: vec2 from swizzled SMEM (32 threads × vec2 bf16)
        ...
        # Vec4 views for vectorized stores (warps 1-8, 8×1 layout)
        ...
        fused_kernel123(
            ...
            tiled_copy_qk_s2r, tiled_copy_qk_k1,
