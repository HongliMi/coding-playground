"""
Fused K1+K2+K3 Pure Memory Bandwidth Test (with g_cumsum fp32 SMEM).

Each block processes 4 chunks with TMA double buffering.
Pure copies through SMEM to measure theoretical GMEM bandwidth.
Extra fp32 SMEM buffer (g_cumsum) to increase SMEM pressure / lower occupancy.

Grid: (NT/4, H, B)
Block: 256 threads (8 warps)
SMEM: ~128KB (q+k+g × [64,128] bf16 × 2 stages + g_cumsum [64,128] fp32)

Per-chunk operations:
  g (bf16 SMEM) → g_cumsum (fp32 SMEM) → kg  [B,T,H,K] bf16
  k → k_scaled            [B,T,H,K] bf16
  q → q_scaled            [B,T,H,K] bf16
    ...
    for chunk_iter in cutlass.range_constexpr(CHUNKS_PER_BLOCK):
        ...
        for ri in cutlass.range_constexpr(ROWS_PER_WARP):
            ...
            for vi in cutlass.range_constexpr(VEC):
                ...
                k_val = csK[row, c]
                mKscaled[i_b, t, i_h, c] = k_val
                if vi < K_HALF // 32:
                    mAkk[i_b, t, i_h, c] = k_val

                mKg[i_b, t, i_h, c] = sGcum[row, c].to(cutlass.BFloat16)
...
def make_host_function(B, NT, H):
    ...
    def host_fn(mQ, mK, mG, mKscaled, mKg, mQscaled, mGkLast, mAqk, mAkk):
        ...
        g_cumsum_layout = cute.make_layout((BT, K_DIM), stride=(K_DIM, 1))

        smem_size = BT * K_DIM * 2 * 6 + BT * K_DIM * 4 + 256
        ...
        fused_memory_kernel(
            ...
            smem_layout_3d, g_cumsum_layout, _NT,
        ).launch(
...
def test():
    ...
    print("=" * 60)
    smem_kb = (BT * K_DIM * 2 * 6 + BT * K_DIM * 4) / 1024
    print(f"B={B}, T={T}, H={H}, K={K}, NT={NT}")
    print(f"Grid: ({NT // CHUNKS_PER_BLOCK}, {H}, {B}), Block: {THREADS}")
    print(f"SMEM: {smem_kb:.0f} KB (3 × [{BT}×{K_DIM}] bf16 × {NUM_STAGES} stages + g_cumsum [{BT}×{K_DIM}] fp32)")
    ...
    print("\n[Correctness]")
    check("k_scaled == k", k_scaled, k)
    check("q_scaled == q", q_scaled, q)
    check("kg == g (bf16->fp32->bf16)", kg, g)
