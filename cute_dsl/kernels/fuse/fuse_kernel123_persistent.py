...
# Default: 65536 / 1024 threads = 64 regs/thread
# WG0-3 (warps 0-15):  K1+TMA  → alloc(72)   K1 needs more regs (loop-carried state)
# WG4-6 (warps 16-27): MMA     → dealloc(56)  MMA accumulators re-init each chunk
# WG7   (warps 28-31): Store   → dealloc(24)
# Total: (4×72 + 3×56 + 1×24) × 128 = 61440 ≤ 65536
NUM_REGS_K1 = 72
NUM_REGS_MMA = 56
NUM_REGS_STORE = 24
    ...
    for work_id in for_generate(block_id, total_cgs, NUM_SMS):
        ...
        if warp_idx < NUM_K1_TMA_WARPS:
            ...
            if warp_idx == 0:
                ...
                gQ_pf = cute.local_tile(tma_tensor_Q, (BT, K_DIM, 1, 1), (0, 0, pf_i_bnt, i_h))
                ...
                gK_pf = cute.local_tile(tma_tensor_K, (BT, K_DIM, 1, 1), (0, 0, pf_i_bnt, i_h))
                ...
                gG_pf = cute.local_tile(tma_tensor_G, (BT, K_DIM, 1, 1), (0, 0, pf_i_bnt, i_h))
...
def make_host_function(B, NT, H):
    ...
    def host_fn(mQ, mK, mG, mA_log, mBeta, scale,
        ...
        _total_cgs = (_NT // CHUNKS_PER_BLOCK) * _H * _B
        _grid_x = min(NUM_SMS, _total_cgs)
