...
def fused_kernel123(
    ...
    if warp_idx >= 1 and warp_idx < 1 + NUM_K1_WARPS:
        ...
        for chunk_iter in cutlass.range_constexpr(CHUNKS_PER_BLOCK):
            ...
            for vi in cutlass.range_constexpr(VEC): #mhl-debug
            ...
            for j in cutlass.range_constexpr(PARTIAL_COLS_PER_WARP // 4):
                ...
                sPartialLast[row_in_prefix, col] = val #mhl-debug
            ...
            for ri in cutlass.range_constexpr(ROWS_PER_K1_WARP):
                ...
                for vi in cutlass.range_constexpr(VEC):
                    ...
                    k_val = csK[row, c].to(cutlass.Float32)
                    q_val = csQ[row, c].to(cutlass.Float32)
        ...
        for chunk_iter in cutlass.range_constexpr(CHUNKS_PER_BLOCK):
            ...
            for ri in cutlass.range_constexpr(ROWS_PER_STORE_WARP):
                ...
                for vi in cutlass.range_constexpr(BT // 32):
                    c = vi * 32 + lane_id
                    mAqk[i_b, t, i_h, c] = csQ[row, c]
                    mAkk[i_b, t, i_h, c] = csK[row, c]
...
def make_host_function(B, NT, H):
    ...
    def host_fn(mQ, mK, mG, mA_log, mBeta, scale,
        ...
        # Q+K+G bf16 × 2 stages + g_cumsum fp32 × 2 stages
        # + sPartialLast [2,8,32,2] fp32 + mbarriers
        smem_size = (BT * K_DIM * 2 * 3 * NUM_STAGES
                     + BT * K_DIM * 4 * NUM_STAGES
                     + K1_COL_GROUPS * K1_ROW_GROUPS * 32 * VEC * 4
                     + 256)
