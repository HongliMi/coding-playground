...
def kda_Akk_kernel(
    ...
    g_smem_layout,               # 3D Layout without swizzle: (BC, BK, NUM_STAGES)
    ...
    # g: (BC, BK, NUM_STAGES) without swizzle - can use direct indexing
...
def run_kda_Akk(
    ...
    # =============== SMEM layouts ===============
    # G: Simple row-major layout WITHOUT swizzle, add 8-col padding to avoid bank conflict
    G_PAD = 0
    g_smem_layout_2d = cute.make_layout((BC, BK), stride=(BK + G_PAD, 1))
    g_smem_layout = cute.make_layout((BC, BK, NUM_STAGES), stride=(BK + G_PAD, 1, BC * (BK + G_PAD)))
    ...
    # G: TMA without swizzle
    tma_atom_g, tma_tensor_g = cpasync.make_tiled_tma_atom(
        tma_load_op, g_view, g_smem_layout_2d, (BC, BK), num_multicast=1
    )
    ...
    # g: 16 * (64+8) * NUM_STAGES * 4 bytes (no swizzle, row-major, 8-col padding)
    ...
    smem_bytes = (4 * BC * (BK + G_PAD) * NUM_STAGES +           # sG (float32, 8-col padding)
    ...
    kda_Akk_kernel(
        ...
        tma_atom_k,
        tma_tensor_k,
        g_smem_layout,       # 3D: (BC, BK, NUM_STAGES) - no swizzle
