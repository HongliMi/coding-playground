...
def kda_Akk_kernel(
    ...
    Akk_tensor: cute.Tensor,     # (B, T, H, K) output (debug: write back loaded g)
    Aqk_tensor: cute.Tensor,     # (B, T, H, K) output (debug: write back loaded q)
    ...
    - We use cp.async pipeline to load g/q/k into smem, then write back the FULL tiles
    ...
    T_full, K_full = gG_batch.layout.shape
    ...
    for it in range(total_tiles):
        ...
        k_abs_base = k_tile * BK

        # Write Akk/Aqk: [BC rows, BK cols] for this (t_sub, k_tile)
        linear = tidx
        while linear < (BC * BK):
            r = linear // BK
            c = linear - r * BK
            ...
            t_abs = t_abs_base + r
            k_abs = k_abs_base + c
            if t_abs < T_full and k_abs < K_full:
                Akk_tensor[(i_b, t_abs, i_h, k_abs)] = sG[(r, c, stage)]
                Aqk_tensor[(i_b, t_abs, i_h, k_abs)] = sQK[(r, c, stage)]
...
if __name__ == "__main__":
    ...
    # Test parameters
    # NOTE: For debug writeback, Akk/Aqk are full-size (B, T, H, K).
    ...
    # - Akk: (B, T, H, K) fp32   <- g
    # - Aqk: (B, T, H, K) fp16   <- q
    Akk = torch.empty(B, T, H, K, dtype=torch.float32, device='cuda')
    Aqk = torch.empty(B, T, H, K, dtype=torch.float16, device='cuda')
    ...
    max_diff_g = (Akk - g).abs().max().item()
    max_diff_q = (Aqk - q).abs().max().item()
    print(f"\nCorrectness:")
    ...
    # - Write: Akk(fp32) + Aqk(fp16)        -> full K
    read_bytes = B * T * H * K * (4 + 2 + 2)
    write_bytes = B * T * H * K * (4 + 2)
